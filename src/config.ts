...
const ConfigSchema = z.object({
  ...
  // Directory the deepseek_edit tool is allowed to read/write within.
  // Every path it touches is resolved against this root and rejected if it
  // escapes it — this is the sandbox boundary, not a convenience default.
  editWorkspace: z.string().min(1).default(process.cwd()),
  // Hard ceiling on a single file deepseek_edit will read/write, to keep
  // the model's context bounded and avoid silently truncated edits.
  editMaxFileBytes: z.number().positive().default(400_000),
});
...
export function loadConfig(): Config {
  const raw = {
    ...
    httpAllowedHosts: process.env.HTTP_ALLOWED_HOSTS
      ...
      : undefined,
    editWorkspace: process.env.DEEPSEEK_WORKSPACE || process.cwd(),
    editMaxFileBytes: process.env.DEEPSEEK_EDIT_MAX_FILE_BYTES
      ? parseInt(process.env.DEEPSEEK_EDIT_MAX_FILE_BYTES, 10)
      : 400_000,
  };
