...
async function main() {
  ...
  if (config.transport === 'http') {
    ...
    console.error(
      '[DeepSeek MCP] Available tools: deepseek_chat (sessions, fallback), deepseek_fim (fill-in-the-middle), deepseek_sessions, deepseek_review (read-only), deepseek_edit (scoped SEARCH/REPLACE apply)'
    );
